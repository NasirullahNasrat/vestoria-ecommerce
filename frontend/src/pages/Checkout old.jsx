...
const StripePaymentForm = ({ onSubmit, processing }) => {
  ...
  if (!stripe || !elements) {
    return (
      <div className="alert alert-danger">
        Payment processing is currently unavailable. Please try another payment method.
      </div>
    );
  }
...
const Checkout = () => {
    ...
    switch(step) {
      ...
      case 3: // Payment
        return paymentMethod.type && 
              (paymentMethod.type !== 'credit' || paymentMethod.details?.id);
      default:
        return true;
    }
  };
    ...
    <div className="card-body">
      ...
      <div className="my-3">
        <div className="form-check">
          ...
          <label className="form-check-label d-flex align-items-center" htmlFor="credit">
            <FaRegCreditCard className="me-2" /> Credit/Debit Card
          </label>
        </div>
        <div className="form-check">
          <input
      ...
      {paymentMethod.type === "credit" && (
        stripePromise ? (
          ...
          <div className="alert alert-danger">
            Credit card payments are currently unavailable. Please choose another payment method.
          </div>
        )
      )}
        ...
        <div className="row">
          ...
          <div className="col-md-6">
            ...
            <div className="card mb-3">
              ...
              <div className="card-body">
                <p>
                  {paymentMethod.type === "credit" && "Credit/Debit Card"}
                  {paymentMethod.type === "paypal" && "PayPal"}
                  {paymentMethod.type === "credit" && paymentMethod.details?.last4 && (
                    <>
                      <br />**** **** **** {paymentMethod.details.last4}
                    </>
                  )}
                </p>
              </div>
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response?.id) {
        // Process payment if not PayPal (PayPal would have been handled via redirect)
        if (paymentMethod.type === 'credit' && response.payment_intent_client_secret) {
      ...
      toast.error(error.message || "Order failed. Please try again.", { id: 'order-submission' });
    }
  };
